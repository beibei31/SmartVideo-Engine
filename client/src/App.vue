  ...
  <div class="app-stage">
    ...
    <header class="navbar">
      <div class="nav-content">
        <div class="brand">
          <span class="brand-do">Smart</span>
            ...
            <div v-if="sidebar.type === 'ai' && timelineItems.length > 0" class="timeline-guide">
              ...
              <div class="timeline-list">
                <button
                    ...
                    :key="`${timelineItem.startTime}-${index}`"
                    class="timeline-node"
                    @click="jumpTo(timelineItem.startTime)"
                >
                  <span class="timeline-dot"></span>
                  <span class="timeline-time">{{ formatSeconds(timelineItem.startTime) }}</span>
                  <span class="timeline-copy">
                    <strong>{{ timelineItem.topic }}</strong>
                    <small>{{ timelineItem.summary }}</small>
                  </span>
                </button>
              </div>
            </div>
            <div v-else-if="sidebar.type === 'ai'" class="markdown-content" v-html="renderedMarkdown"></div>
...
const parseTimeline = (content) => {
  ...
  try {
    ...
    return parsed
        .map(item => ({
          startTime: Number.parseInt(item.startTime, 10),
          topic: String(item.topic || '片段导览'),
          summary: String(item.summary || '')
        }))
        .filter(item => Number.isFinite(item.startTime) && item.startTime >= 0)
  } catch (e) {
    return []
  }
}
...
const jumpTo = async (startTime) => {
  ...
  try {
    ...
    console.warn('Video play was blocked by browser policy.', e)
  }
}
...
const aiAnalyze = async (id) => {
  ...
  try {
    ...
    // 4. 【关键逻辑】检查后端返回的文本或 HTTP 状态码
    // 429 = Token 配额耗尽，或其他包含 ⚠️/❌/算力已耗尽 的响应
    if (res.status === 429 || text.includes("⚠️") || text.includes("❌") || text.includes("算力已耗尽")) {
      // 弹窗提示错误
      showMsg(text, true)
      // 关闭侧边栏，因为任务其实没开始
...
const startPolling = (id, type) => {
  ...
  const timer = setInterval(async () => {
    ...
    if (type === 'ai') {
      ...
      const isSuccess = text.includes("##") || parseTimeline(text).length > 0;
      // 条件2: 失败 (包含错误关键词)
...
.timeline-guide { display: flex; flex-direction: column; gap: 22px; }
.timeline-video { width: 100%; aspect-ratio: 16 / 9; background: #000; border: 1px solid var(--border-tech); border-radius: 8px; }
.timeline-list { position: relative; display: flex; flex-direction: column; gap: 14px; padding-left: 8px; }
.timeline-list::before { content: ""; position: absolute; top: 12px; bottom: 12px; left: 17px; width: 1px; background: linear-gradient(to bottom, var(--accent-lime), rgba(197, 249, 70, 0.08)); }
.timeline-node { position: relative; display: grid; grid-template-columns: 18px 54px 1fr; gap: 12px; width: 100%; padding: 14px 14px 14px 0; background: transparent; border: 1px solid transparent; border-radius: 8px; color: var(--text-main); text-align: left; cursor: pointer; transition: all 0.2s; }
.timeline-node:hover { background: rgba(197, 249, 70, 0.06); border-color: rgba(197, 249, 70, 0.25); transform: translateX(2px); }
.timeline-dot { position: relative; z-index: 1; width: 12px; height: 12px; margin: 6px 0 0 4px; border-radius: 50%; background: var(--bg-card); border: 2px solid var(--accent-lime); box-shadow: 0 0 12px rgba(197, 249, 70, 0.45); }
.timeline-time { color: var(--accent-lime); font-family: monospace; font-size: 0.82rem; line-height: 1.6; }
.timeline-copy { display: flex; flex-direction: column; gap: 5px; min-width: 0; }
.timeline-copy strong { color: var(--text-main); font-size: 1rem; line-height: 1.4; }
.timeline-copy small { color: var(--text-sub); font-size: 0.86rem; line-height: 1.6; }
